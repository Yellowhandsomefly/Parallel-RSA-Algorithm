...
long long rsa_modExp(long long b, long long e, long long m)
{
 ...
 long long en_result = 1;
  ...
  #pragma omp parallel //for reduction(*:en_result) private(t_result)
  { 
	long long i;
	long long t_result = 1;
	int id = omp_get_thread_num();
	nu = omp_get_num_threads();
	for(i = id; i < e; i+=nu){
		//t_result = en_result % m;
		t_result = (t_result * b)% m;
	}
	
	#pragma omp critical
	en_result =  (en_result * t_result) % m;
  }
  return (en_result % m);
}
